			...
			if( $this->Auth->login() ) {
            	...
            	// Retrieve user data
             		...
             		if ( $this->Auth->user() ) {
						...
						$this->Session->setFlash('User '.$this->Session->read('Auth.User.username').' logged in now');
...
//--------------------------------------------------------------------
    function logout() {
    	...
    	if ($this->Session->check('Auth.User.id')) {
    		...
    		$tempUserName = 'You were not logged in, sorry';
    	}
		$this->Auth->logout();
...
//--------------------------------------------------------------------
	...
	function aroset() {
		$this->Acl->Aro->create( array('parent_id' => 1, 'foreign_key' => 1, 'model'=> 'User', 'alias' => null) );
		$this->Acl->Aro->save();
		echo 'aro ok';
		die;
	}
	function permset() {
		...
		//$this->Acl->allow('user', 'controllers/Brands/view');
